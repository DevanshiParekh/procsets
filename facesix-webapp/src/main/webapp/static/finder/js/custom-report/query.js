$(document).ready(function () {
    ...
    $(document).on("click",".edit-icon",function() {
        ...
        window.location.assign("query?proc=edit&id="+clickedEditId);
    });
    ...
    $(document).on("click","#execute-query-btn",function() {
        ...
        $.ajax({
            method: "GET",
            url: "/facesix/rest/fsql/ssql",
            data: {"query":querybox}
        })
        .done(function(e) {
        	e =  JSON.parse(e);
        	alert(" done "+e.success);
            if(e.success==true) {
    ...
    $(document).on("click","#delete-popup-yes",function() {
        ...
        .done(function(e) {
            if(e.success==true) {
                ...
                $(".blkr").removeClass("show");
                window.location.assign("query?proc=list");
            }
            else if(e.success==false) {
                $(".blkr").removeClass("show");
                $(".message-block").text(e.message);
    ...
    $(document).on("click","#update-query-btn",function() {
        ...
        .done(function(e) {
            if(e.success==true) {
                if(nextstep=="yes") {
                    window.location.assign("visual?proc=new&queryid="+e.body.id);
                }
                else if(nextstep=="no") {
                    window.location.assign("query?proc=list");
                }
